 ...
 *
 * Description   : Vector function block tests
 *
 * Author        : codac-dev
 *
...
void testVectorFunction(const std::string type_name, const Type& input, const float64_t& gain, const Type& output) {

  ResourceManager rm;
  FactoryManager fm;
  ServiceGenerator sm(fm);

  BlockConfiguratorTester config("FB_Vector", "Vector<" + type_name + ">", "H.P.T", rm, fm, sm);
  ...
  EXPECT_TRUE(vector_block.process());
  EXPECT_EQ(output_storage->get(), output);
}

template<typename Type>
void testVectorSampleFunction(const std::string type_name, const Type& input, const float64_t& gain, const Type& output) {
  testVectorFunction<Sample<Type>>("Sample<" + type_name + ">",
...
TEST(Vector, Basic) {
  testVectorFunction<float64_t>("float64", 100.0, 0.0, 0.0);
  testVectorFunction<uint64_t>("uint64", 10, 5, 50);
  testVectorSampleFunction<float64_t>("float64", -12.0, 0.5, -6.0);
  testVectorSampleFunction<uint64_t>("uint64", 12, 4, 48);
}
